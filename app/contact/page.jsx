import React from 'react';
import { BiLogoLinkedin } from "react-icons/bi";
import { CiLocationOn } from "react-icons/ci";
import { FaFacebook, FaStackOverflow } from 'react-icons/fa';
import { FaXTwitter } from "react-icons/fa6";
import { IoLogoGithub, IoMdCall } from "react-icons/io";
import { MdAlternateEmail } from "react-icons/md";
import ContactForm from '../components/contactForm';

const personalData = {
    name: "TUSHA",
    profile: '/favicon.ico',
    designation: "Software Developer",
    description: "Passionate Frontend Developer committed to continuous learning and staying up-to-date with the latest technologies and best practices. Experienced building dynamic, responsive web applications with modern frameworks like React, Next.js, and TypeScript. My focus is to deliver high-quality, pixel-perfect solutions.",
    email: 'tushakumar3009@gmail.com',
    phone: '8235028171',
    address: 'Jamshedpur',
    github: 'https://github.com/tusha-30',
    facebook: 'https://www.facebook.com/',
    linkedIn: 'https://www.linkedin.com/in/tusha3009',
    twitter: 'https://twitter.com/',
    stackOverflow: 'https://stackoverflow.com/',
    leetcode: "https://leetcode.com/",
    devUsername: "Tusha",
    resume: "https://drive.google.com/file/d/1jaUwDefACcKXZDaiyq4xthKD8zfUpuJB/view?usp=drive_link"
};

const Contact = () => {
    return (
        <div id="contact" className="my-12 lg:my-16 relative mt-24 text-gray-900 bg-white">
            <div className="hidden lg:flex flex-col items-center absolute top-24 -right-8">
                <span className="bg-gray-200 w-fit text-gray-800 rotate-90 p-2 px-5 text-xl rounded-md">
                    CONTACT
                </span>
                <span className="h-36 w-[2px] bg-gray-200"></span>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
                <ContactForm />
                <div className="lg:w-3/4">
                    <div className="flex flex-col gap-5 lg:gap-9">
                        <p className="text-sm md:text-xl flex items-center gap-3">
                            <MdAlternateEmail
                                className="bg-gray-300 p-2 rounded-full hover:bg-[#16f2b3] hover:scale-110 transition-all duration-300 text-gray-800 cursor-pointer"
                                size={36}
                            />
                            <span>{personalData.email}</span>
                        </p>
                        <p className="text-sm md:text-xl flex items-center gap-3">
                            <IoMdCall
                                className="bg-gray-300 p-2 rounded-full hover:bg-[#16f2b3] hover:scale-110 transition-all duration-300 text-gray-800 cursor-pointer"
                                size={36}
                            />
                            <span>{personalData.phone}</span>
                        </p>
                        <p className="text-sm md:text-xl flex items-center gap-3">
                            <CiLocationOn
                                className="bg-gray-300 p-2 rounded-full hover:bg-[#16f2b3] hover:scale-110 transition-all duration-300 text-gray-800 cursor-pointer"
                                size={36}
                            />
                            <span>{personalData.address}</span>
                        </p>
                    </div>
                    <div className="mt-8 lg:mt-16 flex items-center gap-5 lg:gap-10">
                        <a target="_blank" rel="noopener noreferrer" href={personalData.github}>
                            <IoLogoGithub
                                className="bg-gray-300 p-3 rounded-full hover:bg-[#16f2b3] hover:scale-110 transition-all duration-300 text-gray-800 cursor-pointer"
                                size={48}
                            />
                        </a>
                        <a target="_blank" rel="noopener noreferrer" href={personalData.linkedIn}>
                            <BiLogoLinkedin
                                className="bg-gray-300 p-3 rounded-full hover:bg-[#16f2b3] hover:scale-110 transition-all duration-300 text-gray-800 cursor-pointer"
                                size={48}
                            />
                        </a>
                        <a target="_blank" rel="noopener noreferrer" href={personalData.twitter}>
                            <FaXTwitter
                                className="bg-gray-300 p-3 rounded-full hover:bg-[#16f2b3] hover:scale-110 transition-all duration-300 text-gray-800 cursor-pointer"
                                size={48}
                            />
                        </a>
                        <a target="_blank" rel="noopener noreferrer" href={personalData.stackOverflow}>
                            <FaStackOverflow
                                className="bg-gray-300 p-3 rounded-full hover:bg-[#16f2b3] hover:scale-110 transition-all duration-300 text-gray-800 cursor-pointer"
                                size={48}
                            />
                        </a>
                        <a target="_blank" rel="noopener noreferrer" href={personalData.facebook}>
                            <FaFacebook
                                className="bg-gray-300 p-3 rounded-full hover:bg-[#16f2b3] hover:scale-110 transition-all duration-300 text-gray-800 cursor-pointer"
                                size={48}
                            />
                        </a>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default Contact;
